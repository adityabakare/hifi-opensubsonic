...
async def common_params(
    ...
    if final_u and final_p:
        ...
        if final_p.startswith("enc:"):
            try:
                ...
                logger.debug(
                    "Failed to decode enc: password, using raw value: %s", e)
    ...
    if not user:
        if not final_u:
            raise SubsonicException(
                code=10, message="Required parameter is missing.", fmt=fmt)
        else:
            raise SubsonicException(
                code=40, message="Wrong username or password", fmt=fmt)
...
def _preference_deduplicator(album_list: list) -> list:
    ...
    for title, versions in groups.items():
        ...
        for alb in versions:
            ...
            # Quality base score
            if q in ["HI_RES", "HI_RES_LOSSLESS", "HIRES_LOSSLESS"]:
                score += 10
            elif q == "LOSSLESS":
                score += 8
            elif q == "HIGH":
...
async def fetch_artist_albums(artist_id: int, artist_name: str = "") -> list:
    ...
    try:
        ...
        logger.warning(
            "Failed to fetch albums for artist %s, falling back to search: %s", artist_id, e)
